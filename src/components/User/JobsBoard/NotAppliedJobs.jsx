...
const JobsList = () => {
      ...
      try {
        const token = localStorage.getItem("token");
            const response = await axios.get(
                `${process.env.REACT_APP_API_URL}/user/fetch/notApplied/jobs`,
                {
                    headers: {
  ...
  return (
        ...
        {jobs.map((job) => (
          <Grid item xs={12} xl={5} key={job._id} sx={{margin:{
            ...
            <Card sx={{borderColor:'#d0d0d0', borderStyle:'solid', borderWidth:'1px', borderRadius:'15px'}} >
              <Link to={`/user-dashboard/job-details/${job._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <CardContent>
    ...
    hour12: true, // Use AM/PM format
            ...
            {/* <Card >
              <Link to={`/user-dashboard/job-details/${job._id}`} style={{textDecoration:'none'}}>
